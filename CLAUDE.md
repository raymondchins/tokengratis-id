...
**Code relationship map (generated):** for "which files relate to X" or "is there an existing util/component", read `.claude/agentmap/map.json` (`dependents` / `imports` / `exports`) instead of serial grep. Refresh: `npm run agentmap` or `/agentmap` skill. Generated by agentmap (`@raymondchins/agentmap`, ts-morph), gitignored — not hand-authored.
